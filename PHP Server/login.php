...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    $conn->query($sql);
    $sql = "SELECT id FROM `users` WHERE name= BINARY '$name' AND password= BINARY '$pass'";
    ...
    if ($result->num_rows > 0)
    {
        ...
        $sql = "INSERT INTO `auth_token_table`(`auth_token`, `date_created`, `expiry_date`, `id`) VALUES ('$authToken' , '$date_created' , '$expiry_date' , '$id')";
        if($conn->query($sql)){
            echo $authToken ;
        }
        else{
        ...
        echo "[error] Incorrect UserName or Password : " . $conn->error;
    }
